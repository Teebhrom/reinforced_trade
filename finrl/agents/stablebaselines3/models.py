...
class DRLAgent:
    ...
    def train_model(
        ...
        model.save(f"{config.TRAINED_MODEL_DIR}/{tb_log_name}")
        return model
    ...
    def get_trajectory_load_from_file(model_name, environment, cwd, deterministic=True):
        if model_name not in MODELS:
            raise ValueError(
                f"Model '{model_name}' not found in MODELS."
            )
        ...
        while not done:
            ...
            state, reward, done, terminal, info = environment.step(action)
        ...
        print("reward sum: ", np.sum(rewards_list))
        dataset = {
            'observations': np.array(states_list),
            'next_observations': np.array(next_states_list),
